"""LangChain agent that generates and screenshots a basic HTML page."""
...
def create_langchain_agent(model: str = _DEFAULT_MODEL) -> Any:
    """Create the LangChain HTML lesson generator."""
    from langchain.agents import create_agent  # type: ignore[reportMissingImports]

    from .langchain_tools import capture_screenshot_tool, save_html_document_tool

    return create_agent(
        model=_langchain_model_name(model),
        tools=[
...
def run_html_lesson_langchain_agent(model: str = _DEFAULT_MODEL) -> str:
    """Run the LangChain HTML lesson agent and return its final output."""
    ...
    with _serve_site_directory(_SITE_DIRECTORY) as base_url:
        ...
        try:
            try:
                result = create_langchain_agent(model).invoke(
...
def _capture_index_screenshot() -> str:
    ...
    try:
        ...
        message = "LangChain created index.html, but screenshot capture failed."
